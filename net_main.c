...
int		DEFAULTnet_clientport	= 2600;//R00k
...
int NET_GetMessage (qsocket_t *sock)
{
	...
	if (ret == 0 && sock->driver)
	{
		...
		if (net_time - sock->lastMessageTime > net_connecttimeout.value && sv.active && host_client && sock == host_client->netconnection && !strcmp(host_client->name, "unconnected"))
		{
			NET_Close (sock);
			return -1;
		}
...
void NET_Init (void)
{
	...
	{
		if (i < com_argc-1)
			DEFAULTnet_clientport = Q_atoi (com_argv[i+1]);//Specify the client port-range starting point.
	}
	...
	if ((i = COM_CheckParm ("-port")) || (i = COM_CheckParm ("-udpport")) || (i = COM_CheckParm ("-ipxport")))
	{
		if (i < com_argc-1)
			...
			Con_SafePrintf ("***NET_Init: you must specify a number after -port; using default port 26000.\n\n");
	}
	...
	for (i=0 ; i<net_numsockets ; i++)
	{
		...
		net_freeSockets = s;
		s->disconnected = true;
		s->client_pool = DEFAULTnet_clientport + i;
	}
	// allocate space for network message buffer
	SZ_Alloc (&net_message, NET_MAXMESSAGE);

	Cvar_RegisterVariable (&net_messagetimeout);
	Cvar_RegisterVariable (&net_connecttimeout);	// joe: qkick/qflood protection from ProQuake
	Cvar_RegisterVariable (&hostname);
	Cvar_RegisterVariable (&pq_password);			// JPG 3.00 - password protection
	Cvar_RegisterVariable (&rcon_password);		// joe: rcon password from ProQuake
	Cvar_RegisterVariable (&rcon_server);		// joe: rcon server from ProQuake
...
/*
	...
	if (*my_tcpip_address)
		Con_Printf("TCP/IP address %s\n", my_tcpip_address);//R00k : 'developer 1' wasnt recognized until after this init so Con_Dprint here never worked.
*/
}
