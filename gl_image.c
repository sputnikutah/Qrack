...
#define TGA_MAXCOLORS 16384 //14bit
...
byte *Image_LoadTGA (FILE *fin, char *filename, int matchwidth, int matchheight)
{
	...
	{
		Con_DPrintf (1,"TGA image %s exceeds maximum supported dimensions\n", COM_SkipPath(filename));
		fclose (fin);
		return NULL;
	}
	...
	default:
		Con_DPrintf (1,"Unsupported TGA image %s: Only type 1 (map), 2 (RGB), 3 (mono), 9 (RLEmap), 10 (RLERGB), 11 (RLEmono) TGA images supported\n", COM_SkipPath(filename));
		fclose (fin);
		return NULL;
	}
	...
	default:
		Con_DPrintf (1,"Unsupported TGA image %s: Only 8, 15, 16, 24 or 32 bit images (with colormaps) supported\n", COM_SkipPath(filename));
		fclose (fin);
		return NULL;
	}
	...
	if (mapped)
	{
		...
		default:
			Con_DPrintf (1,"Unsupported TGA image %s: Only 8, 15, 16, 24 or 32 bit colormaps supported\n", COM_SkipPath(filename));
			fclose (fin);
			return NULL;
		}
	...
	for (y=0 ; y<h ; y++)
	{
		...
		for (x=0 ; x<w ; x++)
		{
			...
			default:
				Con_DPrintf (1,"Malformed TGA image %s: Illegal pixel_size '%d'\n", COM_SkipPath(filename), pixel_size);
				fclose (fin);
				Q_free (data);
				if (mapped)
					Q_free (ColorMap);
...
byte *Image_LoadPNG (FILE *fin, char *filename, int matchwidth, int matchheight)
{
	...
	{
		Con_DPrintf (1,"Invalid PNG image %s\n", COM_SkipPath(filename));
		fclose (fin);
		return NULL;
	}
	...
	{
		Con_DPrintf (1,"PNG image %s exceeds maximum supported dimensions\n", COM_SkipPath(filename));
		png_destroy_read_struct (&png_ptr, &pnginfo, NULL);
		fclose (fin);
		return NULL;
	}
	...
	{
		Con_DPrintf (1,"Unsupported PNG image %s: Bad color depth and/or bpp\n", COM_SkipPath(filename));
		png_destroy_read_struct (&png_ptr, &pnginfo, NULL);
		fclose (fin);
		return NULL;
	}
...
byte *Image_LoadJPEG (FILE *fin, char *filename, int matchwidth, int matchheight)
{
	...
	{
		Con_DPrintf (1,"JPEG image %s exceeds maximum supported dimensions\n", COM_SkipPath(filename));
		return NULL;
	}
