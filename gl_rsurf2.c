...
//R00k ah heck, once i increased this, demos recorded with beta versions require this...old demos will still work just as well.
#define MAX_LIGHTMAP_SIZE		10240//Halflife BSP support
...
static	int	allocated[MAX_LIGHTMAPS][LIGHTMAP_BLOCK_WIDTH];
...
#endif
byte	lightmaps[LIGHT_SCALE*MAX_LIGHTMAPS*LIGHTMAP_BLOCK_WIDTH*LIGHTMAP_BLOCK_HEIGHT];
...
//note these are in RGB format, the render sends them to the gpu as GBR
...
#else
	glTexSubImage2D (GL_TEXTURE_2D, 0, 0, theRect->t, LIGHTMAP_BLOCK_WIDTH, theRect->h, GL_RGBA, GL_UNSIGNED_BYTE,	 lightmaps + (lightmapnum * LIGHTMAP_BLOCK_HEIGHT + theRect->t) * LIGHTMAP_BLOCK_WIDTH * LIGHT_SCALE);
#endif
...
update:
	...
	base = lightmaps + fa->lightmaptexturenum * LIGHTMAP_BLOCK_WIDTH * LIGHTMAP_BLOCK_HEIGHT * LIGHT_SCALE;
	base += (fa->light_t * LIGHTMAP_BLOCK_WIDTH + fa->light_s) * LIGHT_SCALE;
	R_BuildLightMap (fa, base, LIGHTMAP_BLOCK_WIDTH * LIGHT_SCALE);
}
...
void GL_CreateSurfaceLightmap (msurface_t *surf)
{
	...
	base = lightmaps + surf->lightmaptexturenum * LIGHTMAP_BLOCK_WIDTH * LIGHTMAP_BLOCK_HEIGHT * LIGHT_SCALE;
	base += (surf->light_t * LIGHTMAP_BLOCK_WIDTH + surf->light_s) * LIGHT_SCALE;
	numdlights = 0;
	R_BuildLightMap (surf, base, LIGHTMAP_BLOCK_WIDTH * LIGHT_SCALE);
}
...
#else
		glTexImage2D (GL_TEXTURE_2D, 0, gl_lightmap_format, LIGHTMAP_BLOCK_WIDTH, LIGHTMAP_BLOCK_HEIGHT, 0, GL_RGB, GL_UNSIGNED_BYTE, lightmaps + i * LIGHTMAP_BLOCK_WIDTH * LIGHTMAP_BLOCK_HEIGHT * 3);
#endif
	}
