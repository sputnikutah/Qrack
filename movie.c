...
cvar_t	capture_dir	= {"capture_dir", "capture", false,false, OnChange_capture_dir};

cvar_t	capture_codec		= {"capture_codec"		, "0"};
cvar_t	capture_fps			= {"capture_fps"		, "30.0"};
cvar_t	capture_console		= {"capture_console"	, "0"};
...
void Movie_Stop (void)
{
	...
	if (moviefile)			//R00k can't close if it failed to be created!
		fclose (moviefile);
}
