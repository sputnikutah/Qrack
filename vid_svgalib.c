...
cvar_t	_windowed_mouse = {"_windowed_mouse","1"}; // dummy for menu.c
cvar_t	m_filter = {"m_filter","1",true};//r00k
cvar_t	m_look		= {"m_look", "1",true};//r00k
...
//      8       9        A       B       C       D       E       F 
	...
	K_ENTER,K_CTRL, '/',    K_F12,  K_ALT,  K_PAUSE,K_HOME, K_UPARROW,
...
void IN_Init (void)
{
	...
	if (UseMouse)
	{
		...
		if ((i = COM_CheckParm("-mdev")) && i + 1 < com_argc)
			mousedev = com_argv[i+1];

		mouserate = 1200;
		if (getenv("MOUSERATE"))
			mouserate = Q_atoi(getenv("MOUSERATE"));
		if ((i = COM_CheckParm("-mrate")) && i + 1 < com_argc)
			mouserate = Q_atoi(com_argv[i+1]);
...
char *VID_ModeInfo (int modenum)
{
	...
	if (modenum == 0)
	{
		sprintf (modestr, "%d x %d, %d bpp", vid.width, vid.height, modes[current_mode].bytesperpixel*8);
		return (modestr);
	}
	else
	{
