...
// by joe
cvar_t		vid_resetonswitch = {"vid_resetonswitch","0", true};
cvar_t		vid_displayfrequency = {"vid_displayfrequency","60", true};
...
int VID_Suspend (MGLDC *dc, m_int flags)
{
	...
	return 0;	// by joe: avoid compiler warning
}
...
{
	if (mode->refreshrate)
		Q_snprintfz (mode->modedesc, sizeof(mode->modedesc), "%dx%d@%d", mode->width, mode->height, mode->refreshrate);
	else
	...
	do
	{
		    ...
		    (nummodes < MAX_MODE_LIST))
		{
			...
			if (ChangeDisplaySettings(&devmode, CDS_TEST | CDS_FULLSCREEN) == DISP_CHANGE_SUCCESSFUL)
			{
				...
				modelist[nummodes].refreshrate = devmode.dmDisplayFrequency;
...
// by joe
	...
	Cvar_RegisterVariable (&vid_displayfrequency);
